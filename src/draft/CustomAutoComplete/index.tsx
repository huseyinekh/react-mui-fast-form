...
const CustomAutoComplete = forwardRef((props: IProps, ref) => {
  ...
  const fetchData = async (val?) => {
    ...
    if (val) {
      ...
      response = await props.getDataFromApi(apiParams ? { ...apiParams, name: val } || { name: val } : {});
    }
    const updatedResponse = (props.dataPath ? response.data[props.dataPath] : response.data) || [];
    ...
    <>
      <MuiAutocomplete
        ...
        getOptionLabel={(option) => {
          return props?.getOptionLabel?.(option) || option.name;
        }}
        noOptionsText='Boşdur'
        renderOption={(renderProps, option) => {
          return (
            <Box component='li' sx={{ minHeight: '40px', borderBottom: '1px solid lightgray' }} {...renderProps}>
        ...
        onChange={(e, val) => {
          ...
          if (!val?.dialogueName) {
            ...
            setValue({...(val||{}),
              [props.optionLabel || 'name']: val?.[props.optionLabel || 'name'] ?? '',
              id: value?.id ?? '',
            });
